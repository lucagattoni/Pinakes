...
- **[`plans/20260805_1721-metadata-as-retrieval-context.md`](plans/20260805_1721-metadata-as-retrieval-context.md)** — whether `title` and `heading_path` are retrieval context or display metadata. It carries the established facts, the agreed order of work, what is decided and what is still open. **Read it before touching chunking, titles or `heading_path`.** Steps 1, 3 and 4 shipped in 0.13.0–0.15.0; what remains is **step 2, the injection experiment**. **It is not one increment and it is not unblocked**: eight adversarial rounds on 20260806 re-scoped it into **2a–2f**, took ten decisions (recorded in its §4 with their rejected alternatives) and measured four conditions that each fail *silently* if missed — the reranker never sees the injection, the RFC corpus has zero token headroom, `.txt` titles are filename stems, and the lexical channel needs `schema_version` 4. **Read §2 and §3 before writing a line**: the revision before that one said the demo KB had no continuation chunks and costed step 2 at "~2 h rebuild + eval", and both were false. §5.4 records the grammar's measurement against 980 RFCs, including **two candidate rules the corpus refused** — read it before proposing a fifth.
