...
- **[`plans/20260805_1721-metadata-as-retrieval-context.md`](plans/20260805_1721-metadata-as-retrieval-context.md)** — whether `title` and `heading_path` are retrieval context or display metadata. It carries the established facts, the agreed order of work, what is decided and what is still open. **Read it before touching chunking, titles or `heading_path`.** Steps 1, 3 and 4 shipped in 0.13.0–0.15.0; what remains is **step 2, the injection experiment**. **It is not one increment**: eight adversarial rounds on 20260806 re-scoped it into **2a–2f**, took every decision its §4 records with their rejected alternatives, and measured four conditions that each fail *silently* if missed — the reranker never sees the injection, the RFC corpus has zero token headroom, `.txt` titles are filename stems, and the lexical channel needs `schema_version` 4. **2a shipped 20260806 06:17 (`86cd403`); 2b is next.** **Read §2 and §3 before writing a line**: three successive revisions each carried a *measured* number that was false — the demo KB having no continuation chunks, step 2 costing "~2 h rebuild + eval", and a prefix reserve of 30 tokens taken from one document whose title happens to be two tokens long. **Any figure there sampled from a single RFC is suspect**; 2a's are sampled from 195. §5.4 records the grammar's measurement against 980 RFCs, including **two candidate rules the corpus refused** — read it before proposing a fifth.
