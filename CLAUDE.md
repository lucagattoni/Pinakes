...
- **[`plans/20260804_1016-template-release.md`](plans/20260804_1016-template-release.md) — started. T1 shipped in 0.17.0, T2 landed 20260807 22:28; T3 is next.** Reviewed, its four decisions taken. **Re-run its Baseline block before trusting any `file:line` in it** — and read T1's and T2's landing commits first: four of the plan's own measurements or specs were wrong (its `notes@1.0` site count, its leg (vii) spec, its claim about which functions leg (vi) would first call, and T2's remedy, which promised a fix no release can deliver), corrected in `5d59ec7` and in T2's ✅ block. D-9 is unanswered in the plan text but answered by the 0.17.0 cut: **the release cuts more than once**, so its name stays in the unbuilt-work table above until the final cut.
