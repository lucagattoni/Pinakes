...
- **[`plans/20260804_1016-template-release.md`](plans/20260804_1016-template-release.md) — started. T1 shipped in 0.17.0; T2 is next.** Reviewed, its four decisions taken. **Re-run its Baseline block before trusting any `file:line` in it** — and read T1's landing commits first: three of the plan's own measurements were wrong (its `notes@1.0` site count, its leg (vii) spec, its claim about which functions leg (vi) would first call), each corrected in `5d59ec7`. D-9 is unanswered in the plan text but answered by the 0.17.0 cut: **the release cuts more than once**, so its name stays in the unbuilt-work table above until the final cut.
