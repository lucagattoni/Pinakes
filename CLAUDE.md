...
- **[`plans/20260804_1016-template-release.md`](plans/20260804_1016-template-release.md) — started. T1 shipped in 0.17.0, T2 in 0.18.0, T3 in 0.19.0, T4 in 0.20.0 (landed 20260808 05:41); T5 (`vector_tier = "sqlite-vec"` stops lying) and T7 (`pnk templates`) remain.** Reviewed, its four decisions taken. **Re-run its Baseline block before trusting any `file:line` in it** — three rows moved again at T4 — and read the landing commits first: **seven** of the plan's own measurements or specs have now been wrong. Five were corrected through T3 (its `notes@1.0` site count, its leg (vii) spec, its claim about which functions leg (vi) would first call, T2's remedy, and T3's exit-code sentence, which contradicted the decision O-2 had already taken). **T4 found two more, both in its own test list**: its named operand test (`…by_a_conflicting_hunk_…`) **cannot discriminate** — a conflicting run refuses and prints nothing, so both operand choices look identical, and the case that separates them is an *already applied* hunk — and its `[budget]` heading predicate is positional, which prints a spending-cap heading for a hunk that changes only the **comments** inside `[budget]`, the shape M3's real drift mostly has. Both corrected in T4's landing commits, with the reasoning. D-9 is unanswered in the plan text but answered by the 0.17.0 cut: **the release cuts more than once**, so its name stays in the unbuilt-work table above until the final cut.
- **[`plans/20260731_1202-open-corrections.md`](plans/20260731_1202-open-corrections.md) has four live items as of 20260808, and they arrived three different ways**: `graph_gate.check_identity` is blind to `chunking` and `--rebuild` never re-chunks a protected paid document (both from *building* 2d, neither visible by reading the code that holds them); a damaged template install escapes as a traceback (from T3's adversarial *reading*); and `--apply` writes nothing on the *same manifest* outcome, so that KB can never record the new reference — **created** by T4, in the increment that closed the CRLF item standing there before it. That last one needs a **decision**, not a correction. **The list refills from use, and building, reading and shipping fill it differently** — add to it when something bites.
