...
- **[`plans/20260804_1016-template-release.md`](plans/20260804_1016-template-release.md) — started. T1 shipped in 0.17.0, T2 in 0.18.0, T3 in 0.19.0 (landed 20260808 04:22); T4 (`pnk upgrade --apply`) is next.** Reviewed, its four decisions taken. **Re-run its Baseline block before trusting any `file:line` in it** — and read T1's, T2's and T3's landing commits first: five of the plan's own measurements or specs were wrong (its `notes@1.0` site count, its leg (vii) spec, its claim about which functions leg (vi) would first call, T2's remedy, which promised a fix no release can deliver, and **T3's own exit-code sentence, which contradicted the decision O-2 had already taken** — resolved 20260807 23:25), corrected in `5d59ec7`, in T2's ✅ block and in T3's exit-code contract block. D-9 is unanswered in the plan text but answered by the 0.17.0 cut: **the release cuts more than once**, so its name stays in the unbuilt-work table above until the final cut.
- **[`plans/20260731_1202-open-corrections.md`](plans/20260731_1202-open-corrections.md) has four live items as of 20260808**: `graph_gate.check_identity` is blind to `chunking` and `--rebuild` never re-chunks a protected paid document (both from *building* 2d, neither visible by reading the code that holds them), plus two from T3's adversarial *reading* — a damaged template install escapes as a traceback, and CRLF survives `--apply` only if T4 handles it. **The list refills from *use*, and building and reading fill it differently** — add to it when something bites.
