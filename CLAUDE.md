...
- **[`plans/20260805_1721-metadata-as-retrieval-context.md`](plans/20260805_1721-metadata-as-retrieval-context.md)** — whether `title` and `heading_path` are retrieval context or display metadata. It carries the established facts, the agreed order of work, what is decided and what is still open. **Read it before touching chunking, titles or `heading_path`.** Steps 1, 3 and 4 shipped in 0.13.0–0.15.0; what remains is **step 2, the injection experiment**. **It is not one increment**: eight adversarial rounds on 20260806 re-scoped it into **2a–2f**, took every decision its §4 records with their rejected alternatives, and measured six conditions that each fail *silently* if missed — three still open (the reranker never sees the injection; the lexical channel needs `schema_version` 4; over-length input is truncated with no warning), three closed by 2a and 2b. **2a shipped 20260806 06:17 (`86cd403`), 2b 08:33 (`fcabc02`), 2c 11:56 (`36f32ce`); 2d is next** — `[chunking] metadata` defaulting to `off`, the `assert_prefix_fits` call, and `embedding_text` at `sync.py:2005`. The prefix, the corpus, the questions and the `before` leg all exist; **the two-leg comparison does not, and `graph_gate.judge` cannot supply it** (§3, *What 2d builds*), nor does the screen's `rerank = "none"` before leg — whose capture must precede the flip. **The golden set is frozen** (`tools/rfc_corpus/questions.yaml`, 110 questions, improvable pool 15): never reword or renumber a question — `id` is what pairs a `before` row with an `after` row. **Budget 46 min per `pnk sync --rebuild`** over its ~43 500 chunks, two legs each at 2d and 2f: start one in the background at the top of a session, never at the end. **Read §2 and §3 before writing a line**: three successive revisions each carried a *measured* number that was false — the demo KB having no continuation chunks, step 2 costing "~2 h rebuild + eval", and a prefix reserve of 30 tokens taken from one document whose title happens to be two tokens long. **Any figure there sampled from a single RFC is suspect**; 2a's, 2b's and 2c's are sampled from 195. §5.4 records the grammar's measurement against 980 RFCs, including **two candidate rules the corpus refused** — read it before proposing a fifth.
