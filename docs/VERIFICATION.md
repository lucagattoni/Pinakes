...
| **a template file entry cannot write outside the KB** — `../../evil.md` and a symlinked directory *in the target*, which is what a KB adopted from an existing directory really presents | T7 | `tests/test_init.py::test_a_template_file_entry_that_escapes_the_target_is_refused` |
