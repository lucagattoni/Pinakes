...
| a refusal names the spelling the index holds when a path differs only in case, `./` or NFC/NFD | G2 | `tests/test_eval.py::test_a_path_wrong_only_in_case_is_refused_with_the_indexed_spelling` |
| the artifact records the retrieval and embedding settings that produced the numbers, not only the corpus | G2 | `tests/test_eval.py::test_the_artifact_records_the_configuration_that_produced_the_numbers` |
