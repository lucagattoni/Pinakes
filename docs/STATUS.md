...
| *the graph release* | Structural edges, the expansion channel (`graph_channel`, default off), `schema_version` 3 — eval-gated. **G1 is on `main`, unreleased:** the evaluation's reproducibility was measured and the tie-ordering defect it found was fixed (see [above](#is-the-evaluation-reproducible--measured-20260801-0035)) — no number moved, and no `schema_version` bump. If G2's headroom measurement fails, G3 and G5 do not start and G1/G2/G4 are cut as their own release, named at the cut |
