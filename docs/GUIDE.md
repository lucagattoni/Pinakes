...
**On every KB that exists today it says this instead** — run 20260807 23:52 against 0.18.0, on a
KB created before the version archive existed:
...
*name*. The content behind that name changed eleven times while the name stayed the same, so no
build can say which of the eleven yours came from, and diffing against a guess would report changes
