...
| this page | the shape of it all, without reading the other three |
...
- **18 releases in 10 days.** `0.1.0` on 20260725; `0.10.0` being cut today.
- **Latest on PyPI: `0.8.0`.** `0.9.0` is tagged and merged but its **upload was refused** — the
  repository rename broke PyPI trusted publishing. Fixable with one setting change by a project
  owner; nothing was burned.
- **Three of four planned releases are done.** The links release shipped across `0.5.0`–`0.6.0`.
- **The graph release is blocked, and has been since 20260801** — not on code, on a *corpus*. Its
  gate was measured and cannot be reached on the demo KB. A real 300-RFC corpus now exists; the
  re-measurement has not been run.
- **The template release is unblocked** — plan written, reviewed, four decisions taken. Nobody has
  started it.
- **Six open corrections** are live, all small, none blocking.

---

## The table

Shipped releases first, oldest to newest. Rows with no number and no date are **not built** — the
project's rule is that a version number belongs to a release only when it is cut.

| Release | Date | Title | What it is |
|---|---|---|---|
| **0.1.0** | 20260725 15:27 | The engine | • ULIDs, sidecars, manifest, SQLite index<br>• `pnk init` / `sync` / `search`<br>• BM25 + vector + rerank, confidence signal<br>• Markdown and text only |
| **0.1.1** | 20260727 14:52 | Research and plumbing | • 14 graph-RAG investigations under `docs/graph/`<br>• `Makefile`, CI free-path gate<br>• No behaviour change |
| **0.1.2** | 20260727 15:25 | The README told the truth | • Four README claims contradicted the code<br>• `[light]` install path fixed |
| **0.1.3** | 20260727 15:40 | The first retrospective | • What v0.1 taught<br>• Three rules promoted into `CLAUDE.md` |
| **0.1.4** | 20260727 21:19 | The PDF build order | • `plans/…-v0.2.md`, I1–I9<br>• Four adversarial review passes before any code |
| **0.2.0** | 20260728 14:05 | PDFs, free path | • `pypdfium2` extractor + layout pipeline<br>• 19-fixture synthetic PDF corpus<br>• Extraction cache, page provenance<br>• `[pdf]` / `[claude]` extras |
| **0.2.1** | 20260728 16:54 | One fact, one home | • `docs/` restructured: GUIDE, CLI, MANIFEST, STATUS<br>• Three stale claims fixed |
| **0.2.2** | 20260728 18:49 | The silent skip, named | • A file matching no `include` glob is now reported<br>• Budget core lands, inert |
| **0.3.0** | 20260729 04:17 | It can spend money — if you ask | • Paid Claude-vision extractor for scanned PDFs<br>• Ledger, caps, reservation/reconciliation<br>• Paid-path allowlist + four gates<br>• `pnk budget` |
| **0.4.0** | 20260729 05:32 | Citing a page | • `docs/paper.pdf:p7` citations, CLI and MCP<br>• `pnk doctor` text-yield check<br>• `docs/VERIFICATION.md` + its gate |
| **0.4.1** | 20260729 07:48 | The sidecar that ate itself | • A sidecar that would not parse was overwritten by a fresh one — losing a permanent ULID<br>• Data-loss bug live since v0.1 |
| **0.5.0** | 20260731 11:27 | Links you can walk | • `pnk links` + `pinakes_links` traversal<br>• Reverse-scan of partner KBs<br>• Second synthetic corpus<br>• `ruamel.yaml`: sidecars round-trip properly |
| **0.6.0** | 20260801 10:51 | Links you can write | • `pnk link` authors a link<br>• `pnk doctor` reports link coverage as a ratio<br>• `[kb] requires_pinakes`<br>• Retrieval made deterministic |
| **0.7.0** | 20260801 12:40 | The measurement that said no | • Per-question eval artifact, stable ids<br>• Golden set 41 → 74 questions<br>• **Deliverable was a number: the graph release's gate cannot be reached** |
| **0.7.1** | 20260801 13:42 | The walk stays in the KB | • `[sources] include` could escape the KB and mint sidecars outside it<br>• Three defects, all live before 0.5.0 |
| **0.8.0** | 20260804 08:40 | Our key, not the SDK's | • **Breaking (paid path):** `PINAKES_ANTHROPIC_API_KEY`, no fallback<br>• Budget defaults raised<br>• STATUS header pinned by a gate<br>• 16 doc claims corrected |
| **0.9.0** | 20260804 12:28 | A site, and a name | • Docs published to a MkDocs site<br>• 31 dead links found and fixed<br>• Repo renamed → project is **Pinakes**<br>• ⚠️ **not on PyPI** — trusted publishing broke |
| **0.10.0** | 20260804 13:35 | You can see it working | • `pnk sync` shows live progress on a terminal<br>• `pnk doctor` no longer tells an interrupted sync to `--rebuild`<br>• Sync timestamps are UTC<br>• 🔄 **being cut right now** |
| | | **Open corrections** | • Six live code/tooling items<br>• Five of six came from *building* the RFC corpus, not from reading code<br>• None blocking |
| | | **The graph release** | • Structural edges, expansion channel, `schema_version` 3<br>• 🚫 **Blocked** — its gate failed on the demo KB (20260801)<br>• Unblocking work is a corpus, not code |
| | | **The graph release, staged** | • PPR channel, the `[ner]` extra<br>• Gate-only: no implementation plan exists, by design<br>• Not scheduled |
| | | **The deep release** | • `pnk ask --deep` — the budgeted agentic loop<br>• Only paid entry point still unbuilt |
| | | **The template release** | • Template ecosystem, `pnk upgrade`, `sqlite-vec` tier<br>• ✅ Plan written and reviewed, decisions taken<br>• Not started |
...
- `pnk init` — stamps a KB from a template, mints its permanent ULID.
- `pnk sync` — walks your documents, mints a sidecar per file, builds `.pinakes/index.db`.
- `pnk search` — BM25 + vector cosine + RRF fusion + optional local rerank.
- `pnk serve` — MCP surface for an agent.
...
No behaviour change — the wheel's code is identical to `0.1.0`.

- **~3,000 lines of graph-RAG research** landed under `docs/graph/`: LightRAG, Microsoft GraphRAG,
  Graphiti, HippoRAG 2, fast-graphrag, Graph-R1, LinearRAG, MiniRAG and more, plus
  `PINAKES_APPROACH.md` synthesising them into a gated build order. Six adversarial passes
  (27 → 7 → 8 → 5 → 1 → 0 findings).
...
- `pnk ask --deep` was described as existing. It does not, to this day.
...
This is where the rule *"verify docs by running the commands they show"* comes from.
...
- `plans/20260727_1543-v0.2.md` — I1–I9, reviewed over **four adversarial passes** before a line was
  written.
...
- New: `GUIDE.md`, `CLI.md`, `MANIFEST.md`, `STATUS.md`, `docs/README.md`.
- **`STATUS.md` becomes the only place in the repo that says what is built.**
- `DESIGN.md` becomes rationale only. README becomes deliberately **version-free**, so it cannot
  drift again.

## 0.2.2 — The silent skip, named · 20260728 18:49

`0.2.0` shipped PDF ingest as its headline feature while the template stamped
`include = ["**/*.md", "**/*.txt"]`. So the real first-run experience was: drop in a PDF, run sync,
read `0 indexed`, get no hint why.
...
- **`pnk doctor` text yield** — reports pages below the fitted floor **per page, not per document**,
  because a 200-page report with eight scanned inserts is exactly the document worth knowing about.
- **`docs/VERIFICATION.md`** — every promise, and the test that holds it, with a gate asserting each
  named test exists. It replaced the v0.2 plan's table, **61 of whose 98 test references did not
  resolve**.
...
and a final one once authoring landed.

## 0.5.0 — Links you can walk · 20260731 11:27

- **`pnk links`** — what a document connects to and what connects to it. Depth capped at 3
  server-side, fan-out capped, no query language, ever.
...
- **`pnk link <source> <target> --rel REL`** — writes one entry into the source document's own
  sidecar and nothing else. Aliases and `self` are resolved to ULIDs **before** anything reaches
  disk, which is what makes a link mean the same thing on someone else's machine.
...
**So the graph release stopped here.** No `schema_version` bump, no forced rebuild for every KB in
existence, for an edge table whose channel could never be licensed.
...
All three live since before `0.5.0`. `pinakes.toml` is committed and shared — cloning a KB and
running sync ran *its author's* `include` against *your* tree.
...
- A gate now pins `STATUS.md`'s own header to `__version__` — it had drifted for four consecutive
  releases while every table below it was updated.
...
- **The project is `Pinakes`; everything you can type is `pinakes`.** The repository moved to
  `github.com/lucagattoni/pinakes`. No identifier changed.
...
## 0.10.0 — You can see it working · 20260804 13:35 · 🔄 being cut
...
- **`pnk doctor` no longer tells an interrupted first sync to `--rebuild`** — a remedy that would
  discard every embedding the interrupted run had already written. It now says `pnk sync`, which
...
Small code and tooling items. Five of six came from *building* the RFC realism corpus rather than
from reading code — which is what that corpus was for. Owned by
...
**Why it stopped:** its gate was measured in `0.7.0` and cannot be reached on `tests/demo-kb`. See
above.

**What has already shipped from it:** G1 (reproducibility) and G4 (`requires_pinakes`) in `0.6.0`;
G2 (the evaluation artifact and the measurement itself) in `0.7.0`. **G3, G5 and G6 do not start.**
...
- The **only paid entry point still unbuilt**. Adding it edits the allowlist, DESIGN §1 and
  `CLAUDE.md` in one commit.
- No plan written. It has been described as "planned" since `0.1.2` — and correcting the README's
  claim that it *existed* was that release's whole point.
...
shipped in `0.2.2` appears in no KB created before it, and nothing detects that divergence — so
existing KBs stay PDF-blind permanently unless their owner edits the manifest by hand. `0.6.0`'s
`requires_pinakes` closed the *diagnosis*; nothing yet closes the gap.
...
on 20260804. T1–T4 are unblocked. **Nobody has started it.**

⚠️ Its measurements are recorded *as of a named commit*, not as properties — `main` moved twice
during the session that wrote it. **Re-run its Baseline block before trusting any line number in it.**

---

## How this project builds

Useful context for reading anything above.
...
- **Unbuilt work is named, never numbered.** For months `v0.3` meant the links release; then `0.2.2`
  ...
  have renumbered ~60 committed references.
...
- **`CHANGELOG.md` and `RETROSPECTIVES.md` are never edited directly** — a change drops a fragment in
  `changelog.d/` or `retro.d/`, spliced at release time. Several agents work here at once, and a
  clean auto-merge is not a correct merge.
