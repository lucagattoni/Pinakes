...
| Headings | **0 of 106 806 chunks carry a `heading_path`** | `in-section`, `parent` and `child` derive **zero edges** (`docs/STATUS.md:342`; `plans/20260731_1202-open-corrections.md` item 3). `strategy = "structural"` uses a Markdown-shaped grammar, RFC section numbering does not match it, and it degrades to size-based chunking **silently** |
